...
function statusText(status) {
  return TRACKING_STATUS_VI[status] || status || "-";
}
...
function parseTelegramIntent(text) {
  ...
  if (command === "/tatthongbao") return { action: "notify_off", code, query };
...
async function runBotTick() {
  ...
  try {
    ...
    console.error("Lỗi bot tick scheduler:", err.message);
  } finally {
    botChecking = false;
  }
}
...
// Khởi chạy các tiến trình quét nền và nhận tin nhắn Telegram
