...
function handleBulkCheck() {
    ...
    if (!rawInput) {
        if (window.showToast) window.showToast('Vui lòng nhập mã ID đơn hàng!', false);
        else alert('Vui lòng điền mã ID!');
        return;
    }
    const formattedOrders = rawInput.replace(/\n/g, ',').split(',').map(id => id.trim()).filter(Boolean).join(',');
    ...
    if (!API_KEY) {
        setTimeout(() => {
            renderBulkTable({ "123456": { "status": "In progress", "start_count": "500", "remains": "100", "charge": "0.05" } });
        }, 800);
        return;
    }
...
function renderBulkTable(apiData) {
    ...
    for (const [orderId, info] of Object.entries(apiData)) {
        if (!info || info.error) {
            const errorMsg = info ? (info.error || "Không tìm thấy thông tin đơn") : "Lỗi phản hồi";
            tableHTML += `
                <tr>
        ...
        switch(String(info.status).toLowerCase()) {
            case 'pending': badgeBg = 'var(--warning-soft)'; badgeText = '⏳ PENDING'; break;
            case 'processing': badgeBg = 'var(--accent-soft)'; badgeText = '⚙️ PROCESSING'; break;
            case 'inprogress': badgeBg = 'var(--brand-soft)'; badgeText = '🔄 IN PROGRESS'; break;
            case 'completed': badgeBg = 'var(--success-soft)'; badgeText = '✅ COMPLETED'; break;
            case 'partial': badgeBg = 'rgba(253, 126, 20, 0.15)'; badgeText = '⚠️ PARTIAL'; break;
            case 'canceled': badgeBg = 'var(--danger-soft)'; badgeText = '❌ CANCELED'; break;
        }
...
window.clearMxhHistory = function() {
    if (confirm('Bạn có chắc chắn muốn xóa toàn bộ lịch sử đơn hàng MXH đã lưu trên trình duyệt này?')) {
        localStorage.removeItem('dgMxhOrders');
        loadMxhHistory();
        if (window.showToast) window.showToast('Đã xóa sạch lịch sử đơn hàng!', true);
    }
};
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    if (!token) {
        ...
        if (window.showToast) {
            window.showToast('Vui lòng đăng nhập trước khi xem lịch sử đơn MXH!', false);
        } else {
