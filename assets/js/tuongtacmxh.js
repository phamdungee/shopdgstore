...
const categoriesData = {
    fb: [
        { id: "fb_cam_xuc", text: "Tăng Cảm Xúc" }, { id: "fb_comment", text: "Tăng Comment" },
        { id: "fb_share", text: "Tăng Share" }, { id: "fb_video", text: "Tăng View Video/Reels" },
        { id: "fb_story", text: "Tăng Views/Cảm Xúc Story" }, { id: "fb_follow", text: "Tăng Follow Fanpage/Trang Cá Nhân" },
        { id: "fb_group", text: "Tăng Thành Viên Group" }, { id: "fb_livestream", text: "Facebook Livestream 🔴" },
        { id: "fb_like_comment", text: "Tăng Like Bình luận" }, { id: "fb_review", text: "Tăng Review Page" },
        { id: "fb_quoc_gia", text: "⭐Facebook Services Theo Quốc Gia" }
    ],
    tt: [
        { id: "tt_like_comment", text: "Tăng Like Comment" }, { id: "tt_repost", text: "Tăng Lượt Đăng Lại" },
        { id: "tt_save", text: "Tăng Lượt Lưu Video" }, { id: "tt_mat_live", text: "Tăng Mắt Livestream 🔴" },
        { id: "tt_like_live", text: "Tăng Like Livestream 🔴" }, { id: "tt_comment_live", text: "Tăng Comment Live 🔴" },
        { id: "tt_pk_live", text: "Tăng Điểm PK 🔴" }
    ],
    ig: [
        { id: "ig_like", text: "Tăng Like" }, { id: "ig_comment", text: "Tăng Comment" },
        { id: "ig_follow", text: "Tăng Follow" }, { id: "ig_view", text: "Tăng View" },
        { id: "ig_share", text: "Tăng Share" }, { id: "ig_story", text: "Instagram Story" },
        { id: "ig_save", text: "Lưu Bài Viết" }, { id: "ig_repost", text: "Tăng Lượt Đăng Lại" },
        { id: "ig_channel", text: "Thành Viên Channel" }, { id: "ig_live", text: "Tăng Mắt Livestream 🔴" },
        { id: "ig_quoc_gia", text: "Instagram Service Theo Quốc Gia" }, { id: "ig_khac", text: "Khác" }
    ],
    yt: [
        { id: "yt_sub", text: "Tăng Người Đăng Ký Kênh" }, { id: "yt_like", text: "Tăng Like Video" },
        { id: "yt_comment", text: "Tăng Bình Luận" }, { id: "yt_share", text: "Tăng Share" },
        { id: "yt_view", text: "Tăng Views" }, { id: "yt_quoc_gia", text: "Youtube Views Theo Quốc Gia" },
        { id: "yt_hours", text: "Watch Hours" }, { id: "yt_live", text: "Live Stream" }
    ]
};

const backupMockData = [
    { "service": 100, "name": "Facebook Like Gói Tốc Độ Cao 👍", "rate": "0.12", "min": "50", "max": "10000", "type": "Default", "platform": "Facebook", "category": "fb_cam_xuc" },
    { "service": 200, "name": "TikTok Tăng Follow Siêu Tốc 🚀 Gói Việt", "rate": "0.45", "min": "100", "max": "50000", "type": "Default", "platform": "TikTok", "category": "tt_like_comment" },
    { "service": 300, "name": "Instagram Tăng Bình Luận Tùy Biến 📝", "rate": "0.85", "min": "10", "max": "5000", "type": "Custom Comments", "platform": "Instagram", "category": "ig_comment" },
    { "service": 400, "name": "YouTube Tăng Người Đăng Ký Trọn Gói 💎", "rate": "12.5", "min": "1", "max": "1", "type": "Package", "platform": "YouTube", "category": "yt_sub" }
];
...
function updateHeaderCartCount() {
    ...
    const quantity = (Array.isArray(items) ? items : []).filter(item => item && item.productSlug).length;
...
function displayNoApiWarning(customMessage) {
    ...
    const defaultMsg = 'Hệ thống đang bảo trì dịch vụ tương tác MXH hoặc chưa cấu hình kết nối. Vui lòng quay lại sau!';
...
function initApp() {
    ...
    if (!token) {
        ...
        if (window.showToast) {
            window.showToast('Vui lòng đăng nhập trước khi sử dụng dịch vụ tương tác MXH!', false);
        } else {
    ...
    const apiBase = window.DG_API_BASE || window.SKYNET_API_BASE || '/api';
    ...
    .then(keyData => {
        if (keyData && keyData.ok && keyData.apiKey) {
            ...
            fetch(TRAMMMO_API_URL, { method: 'POST', body: formData })
                ...
                .then(data => {
                    if (data && !data.error && (Array.isArray(data) || typeof data === 'object')) {
                        originalServicesList = Array.isArray(data) ? data : Object.values(data);
                        ...
                        originalServicesList = originalServicesList.map(item => {
                            if (item && item.name) {
                                ...
                                item.name = item.name.replace(/\s*\|\s*\d+\s*[KkMm]\s*(?=\|)/g, '');
                        ...
                        displayNoApiWarning(data ? (data.error || 'Lỗi phản hồi từ nhà cung cấp API') : 'Lỗi kết nối API');
                    }
                })
                .catch(error => {
                    displayNoApiWarning('Không thể kết nối đến nhà cung cấp dịch vụ tương tác.');
                });
        } else {
            // No API Key in database
            displayNoApiWarning('Quản trị viên chưa cấu hình kết nối API cho dịch vụ này.');
        }
    })
    .catch(err => {
    ...
    document.getElementById('orderPanel')?.addEventListener('input', updateJsonPreview);
}
    ...
    fetch(TRAMMMO_API_URL, { method: 'POST', body: balanceFormData })
        ...
        .then(data => {
            if (data && data.balance) {
                ...
                if (vndEl) vndEl.innerText = Math.round(vndBalance).toLocaleString('vi-VN') + " đ";
                if (usdEl) usdEl.innerText = `($${usdBalance.toFixed(2)} ${data.currency})`;
            }
        }).catch(e => { });
}
...
function updateCategoryDropdown() {
    ...
    let iconHtml = '<span class="category-option-icon"><i class="fa-solid fa-list"></i></span>';
    if (platformSelect === 'fb') {
        iconHtml = '<span class="category-option-icon"><img src="assets/img/ảnh sản phẩm/logo-facebook-vong-tron-xanh-duong-voi-chu-f-mau-trang_1131634-495.avif" style="width:16px; height:16px; border-radius:50%; object-fit:cover;"></span>';
    } else if (platformSelect === 'tt') {
        iconHtml = '<span class="category-option-icon"><img src="assets/img/ảnh sản phẩm/logo-tiktok_705838-12827.avif" style="width:16px; height:16px; border-radius:50%; object-fit:cover;"></span>';
    } else if (platformSelect === 'ig') {
        iconHtml = '<span class="category-option-icon"><svg viewBox="0 0 24 24" width="16" height="16" fill="currentColor" style="display:inline-block; vertical-align:middle;"><path d="M12 2.163c3.204 0 3.584.012 4.85.07 3.252.148 4.771 1.691 4.919 4.919.058 1.265.069 1.645.069 4.849 0 3.205-.012 3.584-.069 4.849-.149 3.225-1.664 4.771-4.919 4.919-1.266.058-1.644.07-4.85.07-3.204 0-3.584-.012-4.849-.07-3.26-.149-4.771-1.699-4.919-4.92-.058-1.265-.07-1.644-.07-4.849 0-3.204.013-3.583.07-4.849.149-3.227 1.664-4.771 4.919-4.919 1.266-.057 1.645-.069 4.849-.069zM12 0C8.741 0 8.333.014 7.053.072 2.695.272.273 2.69.073 7.051.014 8.333 0 8.741 0 12c0 3.259.014 3.668.072 4.948.2 4.358 2.618 6.78 6.98 6.98 1.281.058 1.689.072 4.948.072 3.259 0 3.668-.014 4.948-.072 4.354-.2 6.782-2.618 6.979-6.98.059-1.28.073-1.689.073-4.948 0-3.259-.014-3.667-.072-4.947-.196-4.354-2.617-6.78-6.979-6.98C15.668.014 15.259 0 12 0zm0 5.838a6.162 6.162 0 1 0 0 12.324 6.162 6.162 0 0 0 0-12.324zM12 16a4 4 0 1 1 0-8 4 4 0 0 1 0 8zm6.406-11.845a1.44 1.44 0 1 0 0 2.881 1.44 1.44 0 0 0 0-2.881z"/></svg></span>';
    } else if (platformSelect === 'yt') {
        iconHtml = '<span class="category-option-icon"><img src="assets/img/ảnh sản phẩm/youtube-logo-youtube-icon-transparent-free-png.webp" style="width:16px; height:16px; border-radius:3px; object-fit:contain;"></span>';
    }
    ...
    if (platformSelect && categoriesData[platformSelect]) {
        ...
        catItems.forEach(item => {
            item.addEventListener('click', (e) => {
                ...
                categoryBtn.querySelector('.dropdown-selected-val').innerHTML = item.innerHTML;
...
function getGroupKey(item) {
    ...
    const isFb = fbRegex.test(platform) || fbRegex.test(cat) || fbRegex.test(name);
    const isTt = ttRegex.test(platform) || ttRegex.test(cat) || ttRegex.test(name);
    const isIg = igRegex.test(platform) || igRegex.test(cat) || igRegex.test(name);
    const isYt = ytRegex.test(platform) || ytRegex.test(cat) || ytRegex.test(name);

    if (isFb) {
        if (cat.includes('cảm xúc') || cat.includes('like') || cat.includes('tym') || cat.includes('tim') || name.includes('like') || name.includes('cảm xúc') || name.includes('tym')) {
            if (cat.includes('bình luận') || cat.includes('comment') || name.includes('bình luận') || name.includes('comment')) return 'fb_like_comment';
            return 'fb_cam_xuc';
        }
        if (cat.includes('bình luận') || cat.includes('comment') || name.includes('bình luận') || name.includes('comment')) return 'fb_comment';
        if (cat.includes('share') || cat.includes('chia sẻ') || name.includes('share') || name.includes('chia sẻ')) return 'fb_share';
        if (cat.includes('video') || cat.includes('reels') || cat.includes('view') || cat.includes('lượt xem') || name.includes('video') || name.includes('reels') || name.includes('view')) return 'fb_video';
        if (cat.includes('story') || name.includes('story')) return 'fb_story';
        if (cat.includes('follow') || cat.includes('sub') || cat.includes('theo dõi') || cat.includes('đăng ký') || cat.includes('fanpage') || name.includes('follow') || name.includes('theo dõi') || name.includes('sub') || name.includes('fanpage')) return 'fb_follow';
        if (cat.includes('group') || cat.includes('nhóm') || cat.includes('thành viên') || name.includes('group') || name.includes('nhóm') || name.includes('thành viên')) return 'fb_group';
        if (cat.includes('live') || cat.includes('trực tiếp') || name.includes('live') || name.includes('trực tiếp')) return 'fb_livestream';
        if (cat.includes('review') || cat.includes('đánh giá') || name.includes('review') || name.includes('đánh giá')) return 'fb_review';
        if (cat.includes('quốc gia') || cat.includes('country') || cat.includes('việt') || cat.includes('ngoại') || name.includes('quốc gia') || name.includes('country')) return 'fb_quoc_gia';
        return 'fb_cam_xuc';
    }

    if (isTt) {
        if (cat.includes('like comment') || cat.includes('tim bình luận') || name.includes('like comment') || name.includes('tim bình luận') || name.includes('tym bình luận')) return 'tt_like_comment';
        if (cat.includes('repost') || cat.includes('đăng lại') || name.includes('repost') || name.includes('đăng lại')) return 'tt_repost';
        if (cat.includes('save') || cat.includes('lưu') || name.includes('save') || name.includes('lưu')) return 'tt_save';
        if (cat.includes('mắt') || cat.includes('view live') || name.includes('mắt') || name.includes('view live') || name.includes('mắt live')) return 'tt_mat_live';
        if (cat.includes('like live') || cat.includes('tim live') || name.includes('like live') || name.includes('tim live') || name.includes('tym live')) return 'tt_like_live';
        if (cat.includes('comment live') || cat.includes('cmt live') || name.includes('comment live') || name.includes('cmt live')) return 'tt_comment_live';
        if (cat.includes('pk') || name.includes('pk')) return 'tt_pk_live';
        return 'tt_like_comment';
    }

    if (isIg) {
        if (cat.includes('comment') || cat.includes('bình luận') || name.includes('comment') || name.includes('bình luận')) return 'ig_comment';
        if (cat.includes('follow') || cat.includes('theo dõi') || cat.includes('sub') || name.includes('follow') || name.includes('theo dõi') || name.includes('sub')) return 'ig_follow';
        if (cat.includes('view') || cat.includes('xem') || name.includes('view') || name.includes('xem')) return 'ig_view';
        if (cat.includes('share') || cat.includes('chia sẻ') || name.includes('share') || name.includes('chia sẻ')) return 'ig_share';
        if (cat.includes('story') || name.includes('story')) return 'ig_story';
        if (cat.includes('save') || cat.includes('lưu') || name.includes('save') || name.includes('lưu')) return 'ig_save';
        if (cat.includes('repost') || cat.includes('đăng lại') || name.includes('repost') || name.includes('đăng lại')) return 'ig_repost';
        if (cat.includes('channel') || name.includes('channel')) return 'ig_channel';
        if (cat.includes('live') || name.includes('live')) return 'ig_live';
        if (cat.includes('quốc gia') || cat.includes('country') || name.includes('quốc gia') || name.includes('country')) return 'ig_quoc_gia';
        if (cat.includes('like') || cat.includes('tym') || cat.includes('tim') || name.includes('like') || name.includes('tym') || name.includes('tim')) return 'ig_like';
        return 'ig_khac';
    }

    if (isYt) {
        if (cat.includes('sub') || cat.includes('đăng ký') || name.includes('sub') || name.includes('đăng ký')) return 'yt_sub';
        if (cat.includes('like') || cat.includes('thích') || name.includes('like') || name.includes('thích')) return 'yt_like';
        if (cat.includes('comment') || cat.includes('bình luận') || name.includes('comment') || name.includes('bình luận')) return 'yt_comment';
        if (cat.includes('share') || cat.includes('chia sẻ') || name.includes('share') || name.includes('chia sẻ')) return 'yt_share';
        if (cat.includes('quốc gia') || cat.includes('country') || name.includes('quốc gia') || name.includes('country')) return 'yt_quoc_gia';
        if (cat.includes('hour') || cat.includes('giờ xem') || name.includes('hour') || name.includes('giờ xem')) return 'yt_hours';
        if (cat.includes('live') || cat.includes('stream') || name.includes('live') || name.includes('stream')) return 'yt_live';
        if (cat.includes('view') || cat.includes('xem') || name.includes('view') || name.includes('xem')) return 'yt_view';
        return 'yt_view';
    }
...
function filterServices() {
    ...
    filteredServicesList = originalServicesList.filter(item => {
        ...
        const matchPlatform = !selectedPlatform || info.platform === selectedPlatform;
        const matchCategory = !selectedCategory || itemGroupKey === selectedCategory;
        const matchKeyword = !keyword || item.service.toString().includes(keyword) || item.name.toLowerCase().includes(keyword);
...
function renderTablePage() {
    ...
    if (filteredServicesList.length === 0) {
        container.innerHTML = '<div class="no-result">Không tìm thấy gói dịch vụ phù hợp!</div>';
    ...
    document.getElementById('page-info-display').innerText = `Trang ${currentPage} / ${totalPages}`;
    ...
    paginatedItems.forEach(item => {
        ...
        const isSelectedClass = selectedServiceId === String(item.service) ? ' class="is-selected"' : '';
...
function handleRowSelection(rowEl, serviceId, name, calculatedVndRate, min, max, refill, cancel, type) {
    ...
    handleRowClick(serviceId, name, calculatedVndRate, min, max, refill, cancel, type);
...
function handleRowClick(serviceId, name, calculatedVndRate, min, max, refill, cancel, type) {
    document.getElementById('order-service-display').value = `[ID: ${serviceId}] - ${name}`;
    ...
    document.getElementById('meta-refill').innerText = refill ? "Bảo Hành Hoàn Tiền ✅" : "Không Bảo Hành 🛡️";
...
function generateDynamicFormFields(type) {
    ...
    const noQuantityTypes = ["Package", "Custom Comments Package", "Comment Replies", "Subscriptions"];
...
function calculateTotal() {
    const rateVnd = parseFloat(document.getElementById('order-service-rate').value) || 0;
    ...
    document.getElementById('order-total').innerText = Math.round(total).toLocaleString('vi-VN') + " đ";
}

function switchLinkMode(mode) {
    document.getElementById('tab-one').classList.toggle('active', mode === 'one');
    document.getElementById('tab-many').classList.toggle('active', mode === 'many');
    const fieldWrapper = document.getElementById('link-field-wrapper');
    if (mode === 'one') {
        fieldWrapper.innerHTML = '<input class="sk-input" type="text" id="order-link" placeholder="Nhập liên kết hoặc UID..." oninput="updateJsonPreview()">';
    } else {
        fieldWrapper.innerHTML = '<textarea class="sk-input" id="order-link" rows="3" placeholder="Mỗi dòng nhập 1 liên kết / UID..." oninput="updateJsonPreview()"></textarea>';
    }
    updateJsonPreview();
}
...
function buildCurrentPayload() {
    ...
    const type = document.getElementById('order-service-type').value;
    const link = document.getElementById('order-link') ? document.getElementById('order-link').value.trim() : "";

    if (!serviceId) return null;

    let payload = { key: API_KEY || "demo_key", action: "add", service: parseInt(serviceId), link: link || "chưa nhập link" };
...
function reportMaintenanceToAdmin(serviceId, serviceName, link, quantity, errorMsg) {
    const token = localStorage.getItem('token');
    const apiBase = window.DG_API_BASE || window.SKYNET_API_BASE || '/api';
    if (!token) return;
    ...
    }).catch(err => console.error('Failed to report maintenance to admin:', err));
}
...
function processOrderSubmit() {
    ...
    if (!API_KEY) {
        ...
        saveOrderToHistory(fakeOrderId, serviceName, currentPayload.link, currentPayload.quantity || 0, totalText);
    ...
    fetch(TRAMMMO_API_URL, { method: 'POST', body: orderFormData })
        ...
        .then(data => {
            if (data && data.order) {
                if (window.showToast) {
                    window.showToast(`Đặt hàng thành công! Mã hóa đơn: #${data.order}`, true);
                } else {
                    alert(`Đặt hàng thành công! Mã hóa đơn: #${data.order}\n\nForm gửi đi (Đã ẩn key):\n` + JSON.stringify(alertPayload, null, 2));
                }

                const serviceName = document.getElementById('order-service-display').value;
                const totalText = document.getElementById('order-total').innerText;
                saveOrderToHistory(data.order, serviceName, currentPayload.link, currentPayload.quantity || 0, totalText);
...
function pollSingleOrderStatus(orderId) {
    ...
    if (!API_KEY) {
        setTimeout(() => {
            const mockStatuses = ['Pending', 'Processing', 'In progress', 'Completed'];
...
function updateLiveTrackUI(status, startCount, remains, orderId) {
    ...
    switch (String(status).toLowerCase()) {
        case 'pending': statusEl.style.background = '#ffc107'; statusEl.style.color = '#212529'; break;
        case 'processing': statusEl.style.background = '#17a2b8'; statusEl.style.color = '#fff'; break;
        case 'inprogress': statusEl.style.background = '#007bff'; statusEl.style.color = '#fff'; break;
        case 'completed': statusEl.style.background = '#28a745'; statusEl.style.color = '#fff'; break;
        case 'partial': statusEl.style.background = '#fd7e14'; statusEl.style.color = '#fff'; break;
        case 'canceled': statusEl.style.background = '#dc3545'; statusEl.style.color = '#fff'; break;
    }
    ...
    if (terminalStatuses.includes(String(status).toLowerCase())) {
        ...
        document.getElementById('track-order-msg').innerHTML = `🔒 <strong>Trình theo dõi đã dừng:</strong> Đơn hàng đạt trạng thái cuối (${status}).`;
    }
}
    ...
    if (platformDropdown && platformBtn && platformMenu && platformInput) {
        ...
        items.forEach(item => {
            item.addEventListener('click', (e) => {
                ...
                const btnValContainer = platformBtn.querySelector('.dropdown-selected-val');
